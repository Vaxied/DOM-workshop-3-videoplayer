(function () {
  ...
  const playbackRateSettings = document.querySelector('.playback-rate-container')
...
function addAllEventListeners () {
  ...
  // container listeners
  // videoContainer.addEventListener('keydown', activateKeyboardEvents)
...
function hideVideoControls () {
  ...
  function startListeningInterval () {
    ...
    interval = setInterval(() =>{
      ...
      if (controlsContainer.classList.contains('hidden') || video.paused) {
        return
      }
  ...
  function toggleFullscreen () {
    if (!document.fullscreenElement) {
      fullscreenPromise = document.querySelector('.main-container').requestFullscreen()
  ...
  function saveVideoState () {
    if (video.paused === true) 
      videoPrevState = 'paused'
    else
      videoPrevState = 'playing'
    return videoPrevState
  }

  function setToPreviousState () {
    if (videoPrevState === 'playing')
      playVideo()
    else
      pauseVideo()
  }
  ...
  function closeSettings (event) {
    if (event.target.classList.contains('playback-rate-btn'))
      return
    else if (event.target.classList.contains('main-container') && !video.paused) {
      controlsContainer.classList.add('hidden')
      playbackRateSettings.classList.add('hidden')
      return
    }
  ...
  function updateVideoTimer() {
    ...
    // timeCounter.textContent = `${Math.floor(video.currentTime)}:00 / ${Math.floor(video.duration)}:00`
  }
